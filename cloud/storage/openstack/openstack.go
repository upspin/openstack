...
func (s *openstackStorage) list(url string, perPage int) (refs []upspin.ListRefsItem, nextToken string, err error) {
	...
	err = pager.EachPage(func(page pagination.Page) (bool, error) {
		...
		if err != nil {
			return false, errors.E(op, errors.IO, errors.Errorf("Unable to extract info from page of container %q: %s", s.container, err))
		}
		for _, o := range objs {
		...
		if err != nil {
			return false, errors.E(op, errors.IO, errors.Errorf("Unable to get next page URL for container %q: %s", s.container, err))
		}
		nextToken = token
